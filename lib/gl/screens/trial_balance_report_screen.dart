...
class _TrialBalanceReportScreenState extends State<TrialBalanceReportScreen> {
  ...
  Future<Uint8List> _generatePdf(PdfPageFormat format) async {
    ...
    if (!_showDimensions) {
      conditions.add("ไม่แสดงหน่วยงาน/สาขา/โครงการ");
    }
    if (_hideZero) {
              ...
              pw.TableRow(children: [
                pw.Expanded(
                    child: pw.Container(
                        ...
                        alignment: pw.Alignment.center,
                        child: pw.Text(
                            _showDimensions ? "หน่วยงาน/สาขา/โครงการ" : "-",
                            style: pw.TextStyle(
                                fontWeight: pw.FontWeight.bold,
                                fontSize: 10)))),
                pw.Expanded(
        ...
        rows.addAll(displayData.map((row) {
          ...
          if (_showDimensions &&
            ...
            for (var d in dims) {
              ...
              String dimText = [buCode, brCode, pjCode]
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 16),
                    SwitchListTile(
                      title: const Text('แสดงหน่วยงาน/สาขา/โครงการ'),
                    ...
                    const Divider(),
                    SwitchListTile(
                      title: const Text('แสดงผังบัญชี (Hierarchy)'),
...
//     // const String companyName = "บริษัท ตัวอย่าง จำกัด";
//     String companyName =
//         _company != null ? _company!.thaiName : "(ไม่ระบุชื่อบริษัท)";
...
//     String conditionLine = "* ${conditions.join(", ")}";
//     // if (conditionLine.isEmpty) conditionLine = "-";
...
//     // 2. Apply Hide Zero Filter (Fix Issue #2)
//     if (_hideZero) {
//       displayData = displayData.where((row) {
//         double begDr = double.tryParse(row['beg_dr'].toString()) ?? 0;
//         double begCr = double.tryParse(row['beg_cr'].toString()) ?? 0;
...
//         // ถ้าทุกช่องเป็น 0 ให้ซ่อน
//         // ใช้ 0.001 เพื่อป้องกัน floating point error
//         bool isZero = (begDr.abs() < 0.001 &&
//             begCr.abs() < 0.001 &&
//             mvmtDr.abs() < 0.001 &&
//             mvmtCr.abs() < 0.001 &&
...
//     // Calculate Grand Total (Always from ALL details to be accurate)
//     double sumBegDr = 0, sumBegCr = 0;
//     double sumMvmtDr = 0, sumMvmtCr = 0;
//     double sumEndDr = 0, sumEndCr = 0;
...
//             // Table Header (Fix Issue #4 - Centering Headers)
//             // ใช้ Row + Expanded แทน Table Row แรก เพื่อให้จัดกึ่งกลางระหว่าง 2 คอลัมน์ได้ (Flex 3 ครอบคลุม Flex 1.5+1.5)
//             // pw.Row(
//             //   children: [
//             // pw.Expanded(flex: 4, child: pw.Text("รหัส/ชื่อบัญชี", style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10))),
//             pw.Table(columnWidths: {
//               0: const pw.FlexColumnWidth(4),
//               1: const pw.FlexColumnWidth(3),
//               2: const pw.FlexColumnWidth(3),
...
//                                 fontWeight: pw.FontWeight.bold,
//                                 fontSize: 10)))),
//                 // pw.Expanded(flex: 3, child: pw.Center(child: pw.Text("ยอดยกมา", style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10)))),
//                 pw.Expanded(
//                     child: pw.Container(
//                         padding: const pw.EdgeInsets.fromLTRB(1, 3, 1, 3),
//                         decoration: const pw.BoxDecoration(
...
//                                 fontWeight: pw.FontWeight.bold,
//                                 fontSize: 10)))),
//                 // pw.Expanded(flex: 3, child: pw.Center(child: pw.Text("ยอดในงวด", style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10)))),
//                 pw.Expanded(
//                     child: pw.Container(
//                         padding: const pw.EdgeInsets.fromLTRB(1, 3, 1, 3),
//                         decoration: const pw.BoxDecoration(
...
//                                 fontWeight: pw.FontWeight.bold,
//                                 fontSize: 10)))),
//                 // pw.Expanded(flex: 3, child: pw.Center(child: pw.Text("ยอดยกไป", style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10)))),
//                 pw.Expanded(
//                     child: pw.Container(
//                         padding: const pw.EdgeInsets.fromLTRB(1, 3, 1, 3),
//                         decoration: const pw.BoxDecoration(
...
//             }, children: [
//               pw.TableRow(children: [
//                 // pw.Text(_showDimensions ? "หน่วยงาน/สาขา/โครงการ" : "", style: const pw.TextStyle(fontSize: 9, color: PdfColors.grey700)),
//                 pw.Expanded(
//                     child: pw.Container(
//                         padding: const pw.EdgeInsets.fromLTRB(1, 3, 1, 3),
//                         decoration: const pw.BoxDecoration(
...
//                                 fontWeight: pw.FontWeight.bold,
//                                 fontSize: 10)))),
//                 // pw.Align(alignment: pw.Alignment.centerRight, child:
//                 // pw.Text("เดบิต", style: const pw.TextStyle(fontSize: 10))),
//                 pw.Expanded(
//                     child: pw.Container(
//                         padding: const pw.EdgeInsets.fromLTRB(1, 3, 1, 3),
//                         decoration: const pw.BoxDecoration(
...
//                                 fontWeight: pw.FontWeight.bold,
//                                 fontSize: 10)))),
//                 // pw.Align(alignment: pw.Alignment.centerRight, child: pw.Text("เครดิต", style: const pw.TextStyle(fontSize: 10))),
//                 pw.Expanded(
//                     child: pw.Container(
//                         padding: const pw.EdgeInsets.fromLTRB(1, 3, 1, 3),
//                         decoration: const pw.BoxDecoration(
...
//                                 fontWeight: pw.FontWeight.bold,
//                                 fontSize: 10)))),
//                 // pw.Align(alignment: pw.Alignment.centerRight, child: pw.Text("เดบิต", style: const pw.TextStyle(fontSize: 10))),
//                 pw.Expanded(
//                     child: pw.Container(
//                         padding: const pw.EdgeInsets.fromLTRB(1, 3, 1, 3),
//                         decoration: const pw.BoxDecoration(
...
//           }, children: [
//             pw.TableRow(children: [
//               // pw.Text("ยอดรวม", style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10)),
//               pw.Expanded(
//                   child: pw.Container(
//                       padding: const pw.EdgeInsets.fromLTRB(1, 3, 1, 3),
//                       decoration: const pw.BoxDecoration(
...
//                           style: pw.TextStyle(
//                               fontWeight: pw.FontWeight.bold, fontSize: 10)))),
//               // pw.Align(alignment: pw.Alignment.centerRight, child: pw.Text(fmt.format(sumBegDr), style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10))),
//               pw.Expanded(
//                   child: pw.Container(
//                       padding: const pw.EdgeInsets.fromLTRB(1, 3, 1, 3),
//                       decoration: const pw.BoxDecoration(
...
//                           style: pw.TextStyle(
//                               fontWeight: pw.FontWeight.bold, fontSize: 10)))),
//               // pw.Align(alignment: pw.Alignment.centerRight, child: pw.Text(fmt.format(sumBegCr), style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10))),
//               pw.Expanded(
//                   child: pw.Container(
//                       padding: const pw.EdgeInsets.fromLTRB(1, 3, 1, 3),
//                       decoration: const pw.BoxDecoration(
...
//                           style: pw.TextStyle(
//                               fontWeight: pw.FontWeight.bold, fontSize: 10)))),
//               // pw.Align(alignment: pw.Alignment.centerRight, child: pw.Text(fmt.format(sumMvmtDr), style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10))),
//               pw.Expanded(
//                   child: pw.Container(
//                       padding: const pw.EdgeInsets.fromLTRB(1, 3, 1, 3),
//                       decoration: const pw.BoxDecoration(
...
//                           style: pw.TextStyle(
//                               fontWeight: pw.FontWeight.bold, fontSize: 10)))),
//               // pw.Align(alignment: pw.Alignment.centerRight, child: pw.Text(fmt.format(sumMvmtCr), style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10))),
//               pw.Expanded(
//                   child: pw.Container(
//                       padding: const pw.EdgeInsets.fromLTRB(1, 3, 1, 3),
//                       decoration: const pw.BoxDecoration(
...
//                           style: pw.TextStyle(
//                               fontWeight: pw.FontWeight.bold, fontSize: 10)))),
//               // pw.Align(alignment: pw.Alignment.centerRight, child: pw.Text(fmt.format(sumEndDr), style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10))),
//               pw.Expanded(
//                   child: pw.Container(
//                       padding: const pw.EdgeInsets.fromLTRB(1, 3, 1, 3),
//                       decoration: const pw.BoxDecoration(
...
//                           style: pw.TextStyle(
//                               fontWeight: pw.FontWeight.bold, fontSize: 10)))),
//               // pw.Align(alignment: pw.Alignment.centerRight, child: pw.Text(fmt.format(sumEndCr), style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10))),
//               pw.Expanded(
//                   child: pw.Container(
//                       padding: const pw.EdgeInsets.fromLTRB(1, 3, 1, 3),
//                       decoration: const pw.BoxDecoration(
...
//           // Logic for showing Values
//           // ถ้าเป็น Header และ user ไม่เลือก Show Header Totals และไม่เลือก Show Only Headers -> ซ่อนยอด
//           // (ถ้า Show Only Headers ต้องแสดงยอดเสมอ ไม่งั้นรายงานจะโล่ง)
//           final showValue = !isHeader || _showHeaderTotals || _showOnlyHeaders;
...
//           List<pw.TableRow> dimRows = [];
//           // Fix Issue #3: Check dimension_rows and filters
//           // แสดง Dimensions เมื่อ:
//           // 1. User เปิด Switch _showDimensions
//           // 2. ข้อมูล dimension_rows มีค่า
//           // 3. เป็นรายการ Detail (!isHeader) เพราะ Header ไม่ควรมี Dimension
//           // 4. ไม่ได้อยู่ในโหมด Show Only Headers
//           if (_showDimensions &&
//               row['dimension_rows'] != null &&
//               row['dimension_rows'].isNotEmpty && // Check isNotEmpty
//               !isHeader &&
//               !_showOnlyHeaders) {
//             final dims = row['dimension_rows'] as List;
//             for (var d in dims) {
...
//                           labelText: 'ปีบัญชี', border: OutlineInputBorder()),
//                       onChanged: (val) async {
//                         if (val != null) {
//                           setState(() => _selectedYear = val);
//                           await _loadPeriods(val.id);
...
//                             value: null, child: Text("ทุกงวด (ตั้งแต่ต้นปี)")),
//                         ..._periods.map((p) => DropdownMenuItem(
//                             value: p,
//                             child:
//                                 Text("${p.periodNumber} - ${p.periodName}"))),
//                       ],
//                       decoration: const InputDecoration(
//                           labelText: 'งวดเดือน', border: OutlineInputBorder()),
//                       onChanged: (val) => setState(() => _selectedPeriod = val),
//                     ),
//                     const SizedBox(height: 16),
//                     SwitchListTile(
...
//                         setState(() {
//                           _showHierarchy = v;
//                           // Fix Issue #1: If Hierarchy is OFF, disable others visually (handled in onChanged below)
//                           // But we don't necessarily reset the values unless you want to force them off
//                           if (!v) {
//                             // _showHeaderTotals = false; // Optional: Force Off
//                             // _showOnlyHeaders = false;  // Optional: Force Off
//                           }
//                         });
//                       },
//                       contentPadding: EdgeInsets.zero,
//                     ),
//                     // Fix Issue #1: Independent switches, enabled only if Hierarchy is ON
//                     SwitchListTile(
//                       title: const Text('แสดงเฉพาะหัวบัญชี'),
//                       value: _showOnlyHeaders,
//                       onChanged: _showHierarchy
