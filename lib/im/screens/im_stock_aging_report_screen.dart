...
import '../services/im_stock_aging_report_service.dart';
import '../../utils/file_download.dart';

// รายงานอายุสินค้าคงคลัง — มิเรอร์โครงสร้าง im_stock_balance_by_item_report_screen.dart (filter panel, PDF
// preview, Excel export, category-group linear-pass) แต่แยกความหมาย "อายุ" ออกเป็น 2 คอลัมน์ที่ไม่ปนกัน:
//   - Age (days): อายุจริงจาก im_stock_layer.layer_date — เฉพาะสินค้า FIFO/SPECIFIC เท่านั้น
//   - No Movement (days): วันนับจากขายออก/รับเข้าล่าสุด — proxy สำหรับ AVG/STANDARD ที่ไม่มีข้อมูลราย batch
// แถวหนึ่งจะมีค่าแค่คอลัมน์ใดคอลัมน์หนึ่งเสมอ (ว่างอีกคอลัมน์ ไม่ใช่ 0) ดู imStockAgingReportController.js
// สำหรับรายละเอียดที่มาของทั้งสองค่า และเหตุผลที่ต้องแยกกัน (สรุปจากการคุยกับผู้ใช้ก่อนเริ่มพัฒนา)
//
// ขอบเขต: เป็นรายงาน current-state (ยอด/อายุ ณ ปัจจุบันจริง) — as_of_date มีผลแค่จุดอ้างอิงคำนวณจำนวนวัน
// ไม่ใช่การ reconstruct ยอดคงเหลือย้อนหลังแบบ balance-by-item report (ข้อมูลระดับ batch ไม่ได้เก็บย้อนหลังไว้)
//
// Bucket (0-30/31-60/...) คำนวณฝั่ง frontend ล้วนๆ จาก threshold ที่ผู้ใช้แก้ได้ — เปลี่ยน threshold ไม่ต้อง
// เรียก API ใหม่ แค่ label ใหม่จากข้อมูลที่มีอยู่แล้ว

const Map<String, String> _sortLabelsTh = {
  '': 'ไม่ระบุ',
  'qty': 'เรียงตามจำนวน',
  'value': 'เรียงตามมูลค่า',
  'age': 'เรียงตามอายุ/วันไม่เคลื่อนไหว',
...
const Map<String, String> _sortLabelsEn = {
  ...
  'value': 'Sort by Value',
  'age': 'Sort by Age / No-movement',
  'location': 'Sort by Location',
  'lot_serial': 'Sort by Lot/Serial',
};

const List<int> _defaultBucketThresholds = [30, 60, 90, 180];

class ImStockAgingReportScreen extends StatefulWidget {
  const ImStockAgingReportScreen({super.key});

  @override
  State<ImStockAgingReportScreen> createState() => _ImStockAgingReportScreenState();
}

class _ImStockAgingReportScreenState extends State<ImStockAgingReportScreen> {
  final _reportService  = ImStockAgingReportService();
  ...
  final _bucketCtrl     = TextEditingController(text: _defaultBucketThresholds.join(','));
  ...
  String    _locToLabel   = '';
  String    _sortField = ''; // '' | qty | value | age | location | lot_serial
  String    _sortDir   = 'asc';
  ...
  // ─── bucket thresholds (frontend-only — never re-fetches on change) ────────

  List<int> get _bucketThresholds {
    final parsed = _bucketCtrl.text
        .split(',')
        ...
        .toList()
      ..sort();
    return parsed.isEmpty ? _defaultBucketThresholds : parsed;
  }

  String _bucketLabel(int? days) {
    if (days == null) return '';
    final thresholds = _bucketThresholds;
    int lower = 0;
    for (final t in thresholds) {
      if (days <= t) return '$lower-$t';
      lower = t + 1;
    }
  ...
  static int? _intOrNull(dynamic v) => v == null ? null : (v is int ? v : int.tryParse(v.toString()));
  ...
  String _legendLine(bool isEnglish) => isEnglish
      ? 'Age (days) = true batch age for FIFO/Specific items, from receipt date. No Movement (days) = days since last '
        'sale/issue for Average/Standard items (no batch history exists for these) — the two are different metrics, '
        'never both shown on the same row.'
      : 'อายุ (วัน) = อายุจริงของล็อต (เฉพาะสินค้า FIFO/Specific) นับจากวันรับเข้า | ไม่มีการเคลื่อนไหว (วัน) = '
        'จำนวนวันนับจากขายออกล่าสุด (สินค้า AVG/Standard ไม่มีข้อมูลระดับล็อตให้คำนวณอายุจริง) — สองค่านี้คนละความหมาย '
        'จะไม่แสดงพร้อมกันในแถวเดียว';

  String _conditionLine(bool isEnglish) {
    final p = <String>[];
    p.add('${isEnglish ? "As of" : "ณ วันที่"}: ${DateFormat('dd/MM/yyyy').format(_asOfDate)}');
    ...
    p.add('${isEnglish ? "Aging buckets (days)" : "ช่วงอายุ (วัน)"}: ${_bucketThresholds.join(' / ')}');
  ...
  Future<Uint8List> _generatePdf(PdfPageFormat format) async {
    ...
    final legendLine   = _legendLine(isEnglish);
    ...
    final cw = {
      ...
      'item':     pageW * 0.13,
      'costing':  pageW * 0.06,
      'warehouse':pageW * 0.08,
      'lotSerial':pageW * 0.07,
      'location': pageW * 0.11,
      'unit':     pageW * 0.05,
      'qty':      pageW * 0.07,
      'unitCost': pageW * 0.08,
    ...
    final tableHeader = pw.Container(
      ...
      child: pw.Row(children: [
        ...
        cell(cw['costing']!,   isEnglish ? 'Costing'         : 'ต้นทุน',         bold: true),
        ...
        cell(cw['unitCost']!,  isEnglish ? 'Unit Cost'       : 'ต้นทุน/หน่วย',    bold: true, a: pw.TextAlign.right),
        cell(cw['value']!,     isEnglish ? 'Value'           : 'มูลค่า',         bold: true, a: pw.TextAlign.right),
        cell(cw['age']!,       isEnglish ? 'Age (d)'         : 'อายุ (วัน)',      bold: true, a: pw.TextAlign.right),
        cell(cw['noMove']!,    isEnglish ? 'No Move (d)'     : 'ไม่เคลื่อนไหว(วัน)', bold: true, a: pw.TextAlign.right),
        cell(cw['bucket']!,    isEnglish ? 'Bucket'          : 'ช่วงอายุ',        bold: true),
      ]),
    );
    ...
    pw.Widget totalsRow(PdfColor bg, String label, double qty, double value) => pw.Container(
          ...
          child: pw.Row(children: [
            ...
            cell(cw['unitCost']!, '', bold: true),
            cell(cw['value']!,    fmt.format(value),  bold: true, a: pw.TextAlign.right),
            cell(cw['age']!,      '', bold: true),
            cell(cw['noMove']!,   '', bold: true),
            cell(cw['bucket']!,   '', bold: true),
          ]),
        );
    ...
    for (final r in _reportData) {
      ...
      final unitCost = _num(r['unit_cost']).toDouble();
      final value = _num(r['value']).toDouble();
      final ageDays = _intOrNull(r['age_days']);
      final noMoveDays = _intOrNull(r['no_movement_days']);
      curSubtotalQty += qty;
      ...
      content.add(pw.Container(
        ...
        child: pw.Row(children: [
          ...
          cell(cw['costing']!,   imCostingMethodLabel(r['costing_method']?.toString() ?? '', isEnglish)),
          ...
          cell(cw['unitCost']!,  fmt.format(unitCost),    a: pw.TextAlign.right),
          cell(cw['value']!,     fmt.format(value),       a: pw.TextAlign.right),
          cell(cw['age']!,       ageDays?.toString() ?? '',     a: pw.TextAlign.right),
          cell(cw['noMove']!,    noMoveDays?.toString() ?? '',  a: pw.TextAlign.right),
          cell(cw['bucket']!,    _bucketLabel(ageDays ?? noMoveDays)),
        ]),
      ));
      rowIdx++;
    }
  ...
  Future<void> _exportExcel() async {
    ...
    try {
      ...
      _xl(s, 3, 0, '${isEnglish ? "Note" : "หมายเหตุ"}: ${_legendLine(isEnglish)}');

      final hdrs = isEnglish
          ? ['Category', 'Item', 'Costing', 'Warehouse', 'Lot/Serial', 'Location', 'Unit', 'Quantity', 'Unit Cost', 'Value', 'Age (days)', 'No Movement (days)', 'Bucket']
          : ['หมวดหมู่', 'สินค้า', 'ต้นทุน', 'คลัง', 'ล็อต/ซีเรียล', 'ตำแหน่งที่เก็บ', 'หน่วย', 'จำนวน', 'ต้นทุน/หน่วย', 'มูลค่า', 'อายุ (วัน)', 'ไม่เคลื่อนไหว (วัน)', 'ช่วงอายุ'];
      ...
      void writeGroupSubtotal() {
        ...
        _xl(s, row, 7, curSubtotalQty, bg: groupTotBg, bold: true, align: HorizontalAlign.Right);
        _xl(s, row, 8, '', bg: groupTotBg);
        _xl(s, row, 9, curSubtotalValue, bg: groupTotBg, bold: true, align: HorizontalAlign.Right);
        for (int c = 10; c < 13; c++) {
          _xl(s, row, c, '', bg: groupTotBg);
        }
        row++;
      ...
      for (final r in _reportData) {
        ...
        _xl(s, row, 2, imCostingMethodLabel(r['costing_method']?.toString() ?? '', isEnglish));
        _xl(s, row, 3, _warehouseLabel(r, isEnglish));
        _xl(s, row, 4, _lotSerial(r));
        _xl(s, row, 5, _locationPath(r['location_id'] as int?, isEnglish));
        _xl(s, row, 6, _uomName(r, isEnglish));
        _xl(s, row, 7, qty, align: HorizontalAlign.Right);
        _xl(s, row, 8, unitCost, align: HorizontalAlign.Right);
        _xl(s, row, 9, value, align: HorizontalAlign.Right);
        _xl(s, row, 10, ageDays ?? '', align: HorizontalAlign.Right);
        _xl(s, row, 11, noMoveDays ?? '', align: HorizontalAlign.Right);
        _xl(s, row, 12, _bucketLabel(ageDays ?? noMoveDays));
        row++;
      }
      ...
      _xl(s, row, 7, grandQty, bg: totBg, bold: true, align: HorizontalAlign.Right);
      _xl(s, row, 8, '', bg: totBg);
      _xl(s, row, 9, grandValue, bg: totBg, bold: true, align: HorizontalAlign.Right);
      for (int c = 10; c < 13; c++) {
        _xl(s, row, c, '', bg: totBg);
      }
      ...
      final title = isEnglish ? 'IM_Stock_Aging_Report' : 'รายงานอายุสินค้าคงคลัง';
  ...
  void _xl(Sheet s, int r, int c, dynamic v, {ExcelColor? bg, HorizontalAlign? align, bool bold = false}) {
    ...
    cell.value = v is double ? DoubleCellValue(v) : (v is int ? IntCellValue(v) : TextCellValue(v?.toString() ?? ''));
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildDateField(
                  label: isEnglish ? 'As of Date (age reference)' : 'ณ วันที่ (จุดอ้างอิงคำนวณอายุ)',
                  date: _asOfDate,
                  onPick: (d) => setState(() => _asOfDate = d),
                ),
                const SizedBox(height: 4),
                Text(
                  isEnglish
                      ? 'Uses today\'s actual remaining stock — does not reconstruct a past balance.'
                      : 'ใช้ยอดคงเหลือปัจจุบันจริง ไม่ได้คำนวณย้อนกลับยอดในอดีต',
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    labelText: isEnglish ? 'Aging Buckets (days, comma-separated)' : 'ช่วงอายุ (วัน, คั่นด้วยจุลภาค)',
                    border: const OutlineInputBorder(),
                    isDense: true,
                    helperText: isEnglish ? 'Re-labels instantly — no need to regenerate' : 'เปลี่ยนแล้ว label ใหม่ทันที ไม่ต้องประมวลผลซ้ำ',
                    helperMaxLines: 2,
                  ),
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: ['', 'qty', 'value', 'age', 'location', 'lot_serial']
  ...
  Widget build(BuildContext context) {
    ...
    _reportTitle = isEnglish && perm != null && perm.menuNameEn.isNotEmpty
        ? perm.menuNameEn
        : (perm?.menuName ?? (isEnglish ? 'Stock Aging Report' : 'รายงานอายุสินค้าคงคลัง'));
    return Scaffold(
      appBar: AppBar(
...
// ---------------------------------------------------------------------------
// Item picker dialog — มิเรอร์ im_stock_balance_by_item_report_screen.dart
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Location picker dialog — client-side search over the pre-loaded location list
// ---------------------------------------------------------------------------
