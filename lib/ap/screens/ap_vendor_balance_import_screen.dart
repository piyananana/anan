...
class ApVendorBalanceImportScreen extends StatefulWidget {
  final VoidCallback onFieldsChanged;

  const ApVendorBalanceImportScreen({super.key, required this.onFieldsChanged});

  @override
  State<ApVendorBalanceImportScreen> createState() => _ApVendorBalanceImportScreenState();
}

class _ApVendorBalanceImportScreenState extends State<ApVendorBalanceImportScreen> {
  String? _fileName;
  ...
  Future<void> _loadTemplate() async {
    try {
      ...
      final response = await http.get(
        Uri.parse('${AppConfig.apiAp}/ap_vendor_balance/import/template'),
        headers: headers,
      );
  ...
  Future<void> _validate() async {
    ...
    try {
      ...
      final uri = Uri.parse('${AppConfig.apiAp}/ap_vendor_balance/import/validate');
  ...
  Future<void> _confirmImport() async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('ต้องการนำเข้ายอดเจ้าหนี้คงเหลือ $_validRows รายการใช่หรือไม่?'),
        actions: [
          TextButton(
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse('${AppConfig.apiAp}/ap_vendor_balance/import/confirm'),
        headers: headers,
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Row(children: [
          ...
          SizedBox(width: 8),
          Text('นำเข้ายอดเจ้าหนี้คงเหลือ', style: TextStyle(color: Colors.white)),
        ]),
        actions: [
          IconButton(
              ...
              child: Padding(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: Text(_isImporting
                      ? 'กำลังนำเข้า...'
                      : 'ยืนยันนำเข้ายอดเจ้าหนี้คงเหลือ $_validRows รายการ'),
  ...
  Future<void> _downloadTemplate() async {
    try {
      ...
      final headers = await authService.getAuthHeader();
      final response = await http.get(
        Uri.parse('${AppConfig.apiAp}/ap_vendor_balance/import/template/download'),
        headers: headers,
      );
      if (response.statusCode == 200) {
        await downloadFile(response.bodyBytes, 'ap_vendor_balance_template.xlsx');
      } else {
  ...
  Widget _buildTemplateSection() {
    return Card(
      child: ExpansionTile(
        leading: const Icon(Icons.table_chart_outlined, color: Color(0xFF4527A0)),
        ...
        children: [
          Padding(
            ...
            child: Text(
              'ไฟล์เทมเพลตมี 1 sheet คือ "$_sheetName" — 1 แถว = 1 เอกสารค้างชำระของเจ้าหนี้ 1 ราย '
              '(ไม่มีผลกับบัญชีแยกประเภท GL — ใช้สำหรับตั้งยอดในระบบเจ้าหนี้เท่านั้น)\n'
              '★ = จำเป็นต้องระบุ',
          ...
          SingleChildScrollView(
            ...
            child: DataTable(
              columnSpacing: 16,
              headingRowColor: WidgetStateProperty.all(const Color(0xFFEDE7F6)),
              columns: const [
          ...
          SingleChildScrollView(
            ...
            child: DataTable(
              ...
              headingRowColor: WidgetStateProperty.all(const Color(0xFFEDE7F6)),
              columns: const [
                DataColumn(label: Text('doc_code',      style: TextStyle(fontWeight: FontWeight.bold))),
                DataColumn(label: Text('ชื่อประเภทเอกสาร', style: TextStyle(fontWeight: FontWeight.bold))),
              ],
              rows: _docTypes
                  .map((d) => DataRow(cells: [
          ...
          child: SingleChildScrollView(
            ...
            child: DataTable(
              ...
              rows: _validatedData
                  .take(50)
                  .map((r) => DataRow(cells: [
                        DataCell(Text(_fmt(r['vendor_code']))),
                        DataCell(Text(_fmt(r['doc_code']))),
                        DataCell(Text(_fmt(r['doc_no']))),
                        DataCell(Text(_fmt(r['doc_date']))),
                        DataCell(Text(_fmt(r['due_date']))),
                        DataCell(Text(_fmt(r['currency_code']))),
                        DataCell(Text(_fmtNum(r['exchange_rate']))),
                        DataCell(Text(_fmtNum(r['amount']))),
                        DataCell(ConstrainedBox(
                          constraints: const BoxConstraints(maxWidth: 200),
                          child: Text(_fmt(r['description']),
                              overflow: TextOverflow.ellipsis),
                        )),
                      ]))
                  .toList(),
