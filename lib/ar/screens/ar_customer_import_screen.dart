...
class _ArCustomerImportScreenState extends State<ArCustomerImportScreen> {
  ...
  static const _templateColumns = [
    {'key': 'customer_code',      'label': 'รหัสลูกหนี้ (ว่างได้ถ้าอัตโนมัติ)', 'required': false, 'example': 'C001'},
    {'key': 'customer_group_code','label': 'รหัสกลุ่มลูกค้า',           'required': false, 'example': 'RETAIL'},
    {'key': 'old_customer_code',  'label': 'รหัสลูกหนี้เก่า',           'required': false, 'example': 'CA0001'},
    {'key': 'customer_name_th',   'label': 'ชื่อลูกหนี้ (ไทย)',         'required': true,  'example': 'บริษัท ABC จำกัด'},
    ...
    {'key': 'address_alley',            'label': 'ซอย',                    'required': false, 'example': 'สุขุมวิท 11'},
    {'key': 'address_road',             'label': 'ถนน',                    'required': false, 'example': 'สุขุมวิท'},
    {'key': 'address_sub_district',     'label': 'ตำบล/แขวง',             'required': false, 'example': 'คลองเตย'},
    {'key': 'address_district',         'label': 'อำเภอ/เขต',             'required': false, 'example': 'คลองเตย'},
    {'key': 'address_province',         'label': 'จังหวัด',                'required': false, 'example': 'กรุงเทพมหานคร'},
    {'key': 'address_zip_code',         'label': 'รหัสไปรษณีย์',          'required': false, 'example': '10110'},
    {'key': 'contact_name',             'label': 'ชื่อผู้ติดต่อ',         'required': false, 'example': 'คุณสมศักดิ์'},
    {'key': 'phone',                    'label': 'โทรศัพท์',               'required': false, 'example': '02-123-4567'},
    {'key': 'mobile',                   'label': 'มือถือ',                 'required': false, 'example': '081-234-5678'},
    {'key': 'email',                    'label': 'อีเมล',                  'required': false, 'example': 'info@abc.co.th'},
  ];
        ...
        children: [
          ...
          const Padding(
            ...
            child: Text(
              ...
              'รหัสลูกหนี้: ว่างได้ถ้ากลุ่มลูกค้าหรือการตั้งค่าระบบเปิดใช้รหัสอัตโนมัติ',
          ...
          child: SingleChildScrollView(
            ...
            child: DataTable(
              ...
              rows: _validatedData
                  .take(50)
                  .map((r) => DataRow(cells: [
                        DataCell(Text(r['customer_code'] ?? '(อัตโนมัติ)')),
                        DataCell(Text(r['customer_group_code'] ?? '')),
                        DataCell(ConstrainedBox(
                          constraints: const BoxConstraints(maxWidth: 180),
                          child: Text(r['customer_name_th'] ?? '',
                              overflow: TextOverflow.ellipsis),
                        )),
                        DataCell(Text(r['tax_id'] ?? '')),
                        DataCell(Text('${r['credit_term_months'] ?? 0}/${r['credit_term_days'] ?? 30}')),
                        DataCell(Text('${(r['credit_limit'] ?? 0).toStringAsFixed(0)}')),
                        DataCell(Text(r['address_no'] ?? '')),
                        DataCell(Text(r['address_alley'] ?? '')),
                        DataCell(Text(r['address_road'] ?? '')),
                        DataCell(Text(r['address_sub_district'] ?? '')),
