import 'dart:convert';
import 'package:http/http.dart' as http;
import '../../../config/app_config.dart';
import '../../sa/services/auth_service.dart';

class ArReceiptPaymentReportService {
  final String baseUrl = AppConfig.apiAr;
  final AuthService _authService = AuthService();

  Future<List<Map<String, dynamic>>> getReceiptPaymentReport({
    required String dateFrom,
    required String dateTo,
    int? customerGroupId,
    int? salespersonId,
    String? customerCodeFrom,
    String? customerCodeTo,
    String sortBy = 'customer', // 'customer' | 'amount_desc' | 'amount_asc'
  }) async {
    final headers = await _authService.getAuthHeader();
    final params = <String, String>{
      'date_from': dateFrom,
      'date_to':   dateTo,
      'sort_by':   sortBy,
    };
    if (customerGroupId != null)   params['customer_group_id']  = customerGroupId.toString();
    if (salespersonId != null)     params['salesperson_id']      = salespersonId.toString();
    if (customerCodeFrom != null && customerCodeFrom.isNotEmpty) {
      params['customer_code_from'] = customerCodeFrom;
    }
    if (customerCodeTo != null && customerCodeTo.isNotEmpty) {
      params['customer_code_to']   = customerCodeTo;
    }

    final uri = Uri.parse('$baseUrl/ar_receipt_payment_report')
        .replace(queryParameters: params);
    final response = await http.get(uri, headers: headers);
    if (response.statusCode == 200) {
      return List<Map<String, dynamic>>.from(
          jsonDecode(response.body) as List);
    }
    throw Exception('Failed to load AR receipt payment report: ${response.body}');
  }
}
