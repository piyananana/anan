...
class ModuleDocumentService {
  ...
  Future<List<ModuleDocument>> fetchRowsByUserId(int userId) async {
    final headers = await authService.getAuthHeader();
    final response = await http.get(
      Uri.parse('$baseUrl/sa_module_document/user/$userId'),
      headers: headers,
    );

    if (response.statusCode == 200) {
      List jsonResponse = json.decode(response.body);
      return jsonResponse.map((menu) => ModuleDocument.fromJson(menu)).toList();
  ...
  Future<List<ModuleDocument>> fetchRows() async {
    final headers = await authService.getAuthHeader();
    final response = await http.get(
      Uri.parse('$baseUrl/sa_module_document'),
      headers: headers,
    );

    if (response.statusCode == 200) {
      List jsonResponse = json.decode(response.body);
      return jsonResponse.map((rows) => ModuleDocument.fromJson(rows)).toList();
    } else if (response.statusCode == 401) {
      // อาจจะจัดการกรณี Token หมดอายุหรือไม่ถูกต้อง
      // เช่น logout ผู้ใช้และพาไปหน้า Login
      authService.logout();
  ...
  // *** NEW: Add ModuleDocument ***
  Future<ModuleDocument> addRow(ModuleDocument data) async {
    final headers = await authService.getAuthHeader();
    final response = await http.post(
      ...
      body: jsonEncode(<String, dynamic>{
        ...
        'doc_name_thai': data.docNameThai,
        'doc_name_eng': data.docNameEng,
        'parent_id': data.parentId,
        'sort_order': data.sortOrder,
        'is_doc_type': data.isDocType,
        'is_auto_numbering': data.isAutoNumbering,
        'format_prefix': data.formatPrefix,
        'format_separator': data.formatSeparator,
        'format_suffix_date': data.formatSuffixDate,
        'next_running_number': data.nextRunningNumber,
        'running_length': data.runningLength,
        'is_active': data.isActive,
        'sys_module': data.sysModule,
        'sys_doc_type': data.sysDocType,
      }),
    );

    if (response.statusCode == 201) {
      return ModuleDocument.fromJson(json.decode(response.body));
  ...
  // *** NEW: Update ModuleDocument ***
  Future<ModuleDocument> updateRow(ModuleDocument data) async {
    final headers = await authService.getAuthHeader();
    final response = await http.put(
      Uri.parse('$baseUrl/sa_module_document/${data.id}'),
      headers: headers,
      body: jsonEncode(<String, dynamic>{
        'doc_code': data.docCode,
        'doc_name_thai': data.docNameThai,
  ...
  // *** NEW: Delete ModuleDocument ***
  Future<void> deleteRow(int id) async {
    final headers = await authService.getAuthHeader();
    final response = await http.delete(
      Uri.parse('$baseUrl/sa_module_document/$id'),
      headers: headers,
    );
    ...
    if (response.statusCode == 204) {
      ...
      throw Exception('ModuleDocument not found.');
    } else {
  ...
  Future<void> importDataExcel(PlatformFile excelPlatformFile) async {
    ...
    final request = http.MultipartRequest(
      'POST',
      Uri.parse('$baseUrl/sa_module_document/import'),
    );
  ...
  Future<void> exportDataExcel() async {
    final headers = await authService.getAuthHeader();
    final response = await http.get(
      Uri.parse('$baseUrl/sa_module_document/export'),
      headers: headers,
    );
  ...
  // *** NEW: Get ModuleDocument Details (for Edit Form) ***
  // เนื่องจาก API /account/:id/content ไม่ได้คืนค่า is_active, parent_id
  // เราอาจจะต้องสร้าง API ใหม่ หรือรวมข้อมูลจาก ModuleDocument Model และ AccountContent Model ใน Frontend
  ...
  // เพื่อให้ง่ายที่สุด เราจะใช้ข้อมูลจาก ModuleDocument object ที่อยู่ใน TreeView อยู่แล้ว
  // แต่ถ้าต้องการข้อมูลครบถ้วนจาก DB จริงๆ ต้องเรียก API แยก
  Future<Map<String, dynamic>> fetchDetailForEdit(int rowId) async {
    final headers = await authService.getAuthHeader();
    final response = await http.get(Uri.parse('$baseUrl/sa_module_document/$rowId'),
        headers: headers);
    if (response.statusCode == 200) {
